...
from src.dashboard.face_id_engine import face_id_auth_ui
from src.dashboard.face_id_engine import face_id_auth_ui
from src.report_generator import generate_security_report
import sys
from pathlib import Path
...
from src.dashboard.password_policy import check_password_strength
import sys
PROJECT_ROOT_FOR_IMPORT = Path(__file__).resolve().parents[2]
if str(PROJECT_ROOT_FOR_IMPORT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT_FOR_IMPORT))

from src.dashboard.auth_system import create_user, validate_login, check_password_strength
...
TELEGRAM_LIVE_FILE = PROJECT_ROOT / "outputs" / "dashboard_live_clean.csv"
ALERTS_FILE = PROJECT_ROOT / "outputs" / "realtime_alerts.csv"
THREAT_INTEL_FILE = PROJECT_ROOT / "outputs" / "threat_intel_results.csv"
USER_TRACKING_FILE = PROJECT_ROOT / "outputs" / "real_user_tracking.csv"
INCIDENT_CENTER_FILE = PROJECT_ROOT / "outputs" / "incident_center.csv"
...
# =========================
# CyberShieldX Live Data Fix
# =========================
def load_live_telegram_data(limit=200):
    import pandas as pd
    from pathlib import Path

    file_path = Path(__file__).resolve().parents[2] / "outputs" / "telegram_live_dataset.csv"
...
def load_live_incidents(limit=200):
    import pandas as pd
    from pathlib import Path

    file_path = Path(__file__).resolve().parents[2] / "outputs" / "incident_center.csv"
...
def cyber_password_policy(password):
    import re
    ...
    weak_words = [
        ...
        "qwerty", "cybershieldx", "yazeed", "test", "welcome", "kali"
    ]
    ...
    for word in weak_words:
        if word.lower() in password.lower():
            return False, f"Weak password detected: must not contain '{word}'."

    return True, "Strong password accepted."


def get_lockout_file():
    from pathlib import Path
    return Path("outputs/login_lockout.csv")


def load_lockout_state(email):
    import time
    path = get_lockout_file()
    ...
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        parts = line.split(",")
        if len(parts) >= 3 and parts[0].strip().lower() == email.strip().lower():
            return int(parts[1]), float(parts[2])

    return 0, 0


def save_lockout_state(email, attempts, locked_until):
    path = get_lockout_file()
    path.parent.mkdir(exist_ok=True)
    ...
    if path.exists():
        for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
            parts = line.split(",")
            if len(parts) >= 3 and parts[0].strip().lower() == email.strip().lower():
                rows.append(f"{email.lower()},{attempts},{locked_until}")
                found = True
            else:
                rows.append(line)
...
def is_account_locked(email):
    import time
    attempts, locked_until = load_lockout_state(email)
...
def register_failed_login(email):
    ...
    if attempts >= 3:
        ...
        save_lockout_state(email, attempts, locked_until)
        return True, 15 * 60

    save_lockout_state(email, attempts, 0)
    return False, 0


def reset_failed_login(email):
    save_lockout_state(email, 0, 0)
...
# EMERGENCY LIVE TELEGRAM PANEL
st.markdown("## 🔴 LIVE TELEGRAM INCIDENTS - Direct CSV View")
_live_file = PROJECT_ROOT / "outputs" / "dashboard_live_clean.csv"
if _live_file.exists():
    try:
        _live_df = pd.read_csv(_live_file, encoding="utf-8-sig", engine="python", on_bad_lines="skip")
        st.success(f"Live records loaded: {len(_live_df)}")
        st.dataframe(_live_df.tail(20), use_container_width=True)
    except Exception as e:
        st.error(f"Failed to read live CSV: {e}")
else:
    st.warning("Live CSV not found yet. Start live_sync.py and Telegram listener.")
...
DEFAULTS = {
    ...
    "pending_face_mode": None,
    "pending_face_identity": None,
}
...
def render_admin_dashboard():
    ...
    with st.expander("⚖️ Jordanian Law Engine", expanded=False):
        st.markdown("### ⚖️ Jordanian Law Engine - Safe Mode")
        st.info("This engine maps detected cyber incidents to Jordanian cybercrime legal categories.")

        law_text = st.text_area(
            "Enter message/evidence for legal mapping",
            key="safe_jordanian_law_text_area"
        )

        matched_rules = st.text_input(
            "Matched Rules / AI Classification",
            value="blackmail, harassment, defamation, phishing",
            key="safe_jordanian_law_rules"
        )

        if st.button("⚖️ Run Jordanian Law Mapping", use_container_width=True, key="safe_jordanian_law_mapping_btn"):
            text_law = str(law_text or "").lower()
            rules_law = str(matched_rules or "").lower()
            full_law = text_law + " " + rules_law

            if any(x in full_law for x in ["ابتزاز", "صورك", "ادفع", "تدفع", "blackmail", "extortion"]):
                case = "Blackmail / ابتزاز"
                article = "Article 18"
                severity = "High"
                penalty = "Imprisonment not less than 1 year, fine, or both according to Jordanian cybercrime context."
            elif any(x in full_law for x in ["تحرش", "اساءة", "harassment"]):
                case = "Harassment / إساءة أو تحرش"
                article = "Article 15"
                severity = "Medium to High"
                penalty = "Imprisonment or fine according to the severity of the act."
            elif any(x in full_law for x in ["تشهير", "defamation"]):
                case = "Defamation / تشهير"
                article = "Article 11"
                severity = "Medium"
                penalty = "Legal liability based on defamation and reputation damage."
            elif any(x in full_law for x in ["phishing", "رابط", "login", "verify", "otp", "كود"]):
                case = "Phishing / اصطياد إلكتروني"
                article = "Articles 8, 9, 18"
                severity = "High"
                penalty = "Depends on unauthorized access, fraud, or data theft."
            else:
                case = "No clear legal match"
                article = "N/A"
                severity = "Low"
                penalty = "No direct cybercrime mapping detected."

            st.success("Jordanian law mapping completed.")
            st.table({
                "Case": [case],
                "Article": [article],
                "Severity": [severity],
                "Penalty / Legal Meaning": [penalty]
            })

        st.markdown("### 📚 Law Engine Reference Table")
        law_reference = pd.DataFrame([
            {"Case": "Blackmail / ابتزاز", "Article": "Article 18", "Severity": "High"},
            {"Case": "Harassment / إساءة أو تحرش", "Article": "Article 15", "Severity": "Medium to High"},
            {"Case": "Defamation / تشهير", "Article": "Article 11", "Severity": "Medium"},
            {"Case": "Phishing / اصطياد إلكتروني", "Article": "Articles 8, 9, 18", "Severity": "High"},
        ])
        st.dataframe(law_reference, use_container_width=True)
...
if not st.session_state.authenticated and not st.session_state.signin_completed:
    ...
    if st.session_state.get("pending_face_mode"):
        face_identity = st.session_state.get("pending_face_identity") or "default_user"
        face_mode = st.session_state.get("pending_face_mode")

        ok_face = face_id_auth_ui(face_identity, face_mode)

        if ok_face:
            st.session_state.pending_face_mode = None
            st.session_state.pending_face_identity = None
            st.session_state.signin_completed = True
            st.success("Face ID completed. Proceeding to OTP verification...")
            st.rerun()
    ...
    if auth_mode == "Sign in":
        ...
        if st.button("Create Account / Sign in", use_container_width=True, key="device_signin_btn_old_disabled_1"):
            password_to_check = password if "password" in locals() else (p if "p" in locals() else "")
            strong_ok, strong_msg = cyber_password_policy(password_to_check)
            if not strong_ok:
                st.error(strong_msg)
                st.stop()
            ...
            st.session_state.pending_face_mode = "sign_in"
            st.session_state.pending_face_identity = username.strip().lower()

            st.success("Sign in accepted. Proceeding to Face ID enrollment...")
            st.rerun()
        ...
        if st.button("Login to Dashboard", use_container_width=True, key="device_login_btn_old_disabled_3"):
            ...
            if locked:
                st.error(f"Account locked due to multiple failed login attempts. Try again in {remaining} seconds.")
                st.stop()

            strong_ok, strong_msg = cyber_password_policy(password)
            if not strong_ok:
                locked_now, _ = register_failed_login(username)
                if locked_now:
                    st.error("Account locked for 15 minutes after 3 failed login attempts.")
                else:
                    st.error(strong_msg)
                st.stop()
            ...
            st.session_state.device_uid = device_info["device_uid"]
            st.session_state.pending_face_mode = "login"
            st.session_state.pending_face_identity = username.strip().lower()

            st.success("Login accepted. Proceeding to Face ID verification...")
            st.rerun()
    ...
    with col_signin:
        ...
        if st.button("Create Session / Sign in", use_container_width=True, key="device_signin_btn_old_disabled_2"):
            password_to_check = password if "password" in locals() else (p if "p" in locals() else "")
            strong_ok, strong_msg = cyber_password_policy(password_to_check)
            if not strong_ok:
                st.error(strong_msg)
    ...
    with col_login:
        ...
        if st.button("Login to Dashboard", use_container_width=True, key="device_login_btn_old_disabled_4"):
            if not username.strip():
                st.error("Username or email is required.")
                st.stop()
            ...
            log_login_alert(device_info["device_uid"], "login_opened", "success", "Login password verified")
            st.session_state.device_uid = device_info["device_uid"]
            st.session_state.signin_completed = True
...
if not st.session_state.authenticated:
    ...
    with alert_col1:
        if st.button("🔐 Login Alert", use_container_width=True, key="main_login_alert_btn_old_disabled"):
    ...
    with alert_col2:
        if st.button("📝 Sign-in Alert", use_container_width=True, key="main_signin_alert_btn_old_disabled"):
            if not login_uid.strip():
                st.error("UID is required.")
            else:
            ...
            if lockout_active(st.session_state.lockout_until):
                ...
                try:
                    ...
                    if verified:
                        ...
                        user_for_face = st.session_state.get("last_destination", "") or st.session_state.get("auth_username", "") or "default_user"
...
if st.session_state.login_time:
    ...
    st.sidebar.markdown(f"🔐 Session active. Time remaining: **{session_left}s**")
...
if st.button("📄 Generate CyberShieldX PDF Report", use_container_width=True, key="generate_pdf_report_btn"):
    try:
...
# Face ID Direct Camera Section disabled after login
