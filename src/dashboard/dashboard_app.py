    ...
    with st.expander('🛡️ SOC Dashboard', expanded=False):
        ...
        if soc_df.empty:
            ...
            with c_left:
                ...
                risk_col = None
                for c in ["risk", "hybrid_risk", "risk_level", "ai_risk", "rule_risk"]:
                    if c in soc_df.columns:
                        risk_col = c
                        break

                chat_col = "chat_type" if "chat_type" in soc_df.columns else None

                if risk_col:
                    heat_df = soc_df.copy()
                    if chat_col is None:
                        heat_df["source_type"] = "Telegram"
                        chat_col = "source_type"

                    heat_df[chat_col] = heat_df[chat_col].astype(str).replace({"nan": "Unknown", "": "Unknown"})
                    heat_df[risk_col] = heat_df[risk_col].astype(str).replace({"nan": "Low", "": "Low"})

                    heatmap_data = (
                        heat_df.groupby([chat_col, risk_col])
                        .size()
                        .reset_index(name="count")
                    )
                    ...
                    st.warning("Risk Heatmap needs one of these columns: risk, hybrid_risk, risk_level, ai_risk, rule_risk.")
...
if not st.session_state.authenticated and not st.session_state.signin_completed:
    ...
    if auth_mode == "Sign in":
        ...
        if st.button("Create Account / Sign in", use_container_width=True, key="device_signin_btn_old_disabled_1"):
            ...
            create_ok, create_msg = create_user(username, password)
            if not create_ok and "already registered" not in create_msg.lower():
                st.error(create_msg)
                st.stop()
