...
def _old_analyze_hybrid(*args, **kwargs):
...
def analyze_hybrid(*args, **kwargs):
    result = _old_analyze_hybrid(*args, **kwargs)

    text_value = ""
    if args:
        text_value = args[0]
    text_value = kwargs.get("text", text_value)

    if isinstance(result, dict):
        matrix = calculate_5x5_risk(
            text=text_value,
            ai_label=result.get("ai_label", ""),
            rule_risk=result.get("rule_risk", result.get("risk", "Low")),
            ai_risk=result.get("ai_risk", "Low")
        )

        result["likelihood"] = matrix["likelihood"]
        result["impact"] = matrix["impact"]
        result["risk_matrix_score"] = matrix["risk_score"]
        result["quantitative_risk"] = matrix["quantitative"]
        result["qualitative_risk"] = matrix["qualitative"]
        result["risk_formula"] = matrix["formula"]

        result["risk"] = matrix["risk_level"]
        result["final_risk"] = matrix["risk_level"]
        result["hybrid_risk"] = matrix["risk_level"]
        result["risk_level"] = matrix["risk_level"]
        result["score"] = matrix["risk_score"]
        result["hybrid_score"] = matrix["risk_score"]

        if matrix["risk_level"] in ["High", "Critical"]:
            result["alert"] = "RISK_MATRIX_HIGH_ALERT"
